...
while True:
	...
	while (x, y) != goal_pos:
		...
		if not moved:
			if (x, y) in branch2_dict:
				if branch2_dict[(x, y)] != None:
					move(branch2_dict[(x, y)])
					last_time_dir = branch2_dict[(x, y)]
					x = get_pos_x()
					y = get_pos_y()
			elif (x, y) in branch3_dict and len(branch3_dict[(x, y)]) == 0:
				parent_back_dir = opp_dict[move_hist[(x, y)][1]]
				move(parent_back_dir)
				last_time_dir = parent_back_dir
				x = get_pos_x()
				y = get_pos_y()
